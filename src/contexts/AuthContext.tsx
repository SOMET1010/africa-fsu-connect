...
export const AuthProvider = ({ children }: AuthProviderProps) => {
    ...
    const fetchProfile = async (userId: string) => {
        ...
        try {
            ...
            // Fallback : si role est undefined dans profile, utiliser user_metadata.role
            const resolvedRole = data?.role ?? session?.user?.user_metadata?.role ?? 'public';
            setProfile({ ...data, role: resolvedRole });

        } catch (err) {
            logger.error('Error fetching profile:', err, { component: 'AuthContext', action: 'fetchProfile', userId });
            setProfile(null);
        } finally {
