...
export function SimplifiedSidebar() {
    ...
    const isAdminUser =
        profile?.role &&
        ["super_admin", "country_admin", "editor"].includes(profile.role)

    const getNavClassName = (path: string) => {
        const baseClasses =
            "w-full justify-start transition-all duration-200 h-12"
        if (isActive(path)) {
    ...
    return (
        <Sidebar className="border-r border-border bg-card" collapsible={true}>
