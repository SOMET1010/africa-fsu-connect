  ...
  return (
    <div className="container mx-auto px-4 pb-8 animate-fade-in" style={{ contentVisibility: 'auto' }}>
      <div className={cn("text-center", isRTL && "text-right")}>
        <p className={cn("text-xs uppercase tracking-widest mb-4", d ? "text-white/70" : "text-[hsl(var(--nx-text-500))]")}>{title}</p>
        <div className="flex flex-wrap items-center justify-center gap-6">
          ...
          {items.map((partner, i) => (
            <span key={i} className={cn("text-sm font-medium px-4 py-2 rounded-full border", d ? "text-white/90 border-white/20" : "text-[hsl(var(--nx-text-700))] border-[hsl(var(--nx-border))]")}>
              {partner}
            </span>
          ))}
        </div>
