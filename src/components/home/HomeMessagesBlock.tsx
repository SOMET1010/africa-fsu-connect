...
export const HomeMessagesBlock = ({ variant = 'light' }: { variant?: 'light' | 'dark' }) => {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {OFFICIAL_MESSAGES.map((msg, i) => (
            <div
              key={i}
              className={cn("relative rounded-2xl p-8 transition-colors", d ? "bg-white/5 border border-white/10 hover:border-[hsl(var(--nx-gold))]/20" : "bg-[hsl(var(--nx-surface))] border border-[hsl(var(--nx-border))] shadow-[var(--nx-shadow-sm)] hover:border-[hsl(var(--nx-gold))]/20")}
            >
              ...
              <div className={cn("flex items-center gap-4 mb-6", isRTL && "flex-row-reverse")}>
                ...
                <div>
                  <h3 className={cn("font-semibold text-lg", d ? "text-white" : "text-[hsl(var(--nx-text-900))]")}>{msg.title}</h3>
